...
def kp_detection(img, img_temp,i):
    ...
    cv2.imwrite('data/result/kp/temp_add/img'+str(i)+'.jpg', img)
...
def temp_matching(img, img_temp,i):
    ...
    cv2.imwrite('data/result/tm/temp_add/img'+str(i)+'.jpg', img)

if __name__ == '__main__':
    imgs = os.listdir('data/orig/images')
    temps = os.listdir('data/orig/templates_add')
    
    for i in range(len(imgs)):
        img = cv2.imread('data/orig/images/'+imgs[i])
        img_temp = cv2.imread('data/orig/templates_add/'+temps[i])
